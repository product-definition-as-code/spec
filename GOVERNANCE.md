...
So the windows in [CONTRIBUTING.md](CONTRIBUTING.md) are keyed to the constituency instead: the minimum elapsed times begin once an independent implementation or clean-room validator exists (gate 2 in [MATURITY.md](MATURITY.md)), or a listed adopter other than the reference implementation exists (the first of the two adopters gate 3 requires). This is not the suspension under another name. The suspension was a latitude the founding maintainer granted himself, sunset at a version, and it took a governance act to end. This is a published condition inside the rule: it is checkable by anyone, it begins on its own, and no maintainer can extend it. The BDFL bootstrap authority is not exercised here and is not needed.
