...
A second implementer needs nothing from ProductShape source to parse and validate artifacts: these schemas plus the specification chapters are the whole contract. What is still missing for full independent conformance is the executable corpus (see [conformance/](../conformance/)), which is planned.
