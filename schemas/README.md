...
**These JSON Schemas are normative.** They define the serialization contract (version: `v1alpha1`) for every artifact kind and for repository configuration: closed shapes, allowed values and required fields that the specification text describes in prose. Where prose and schema disagree, the schema wins and the prose has a bug.
