...
Product Definition as Code adds that layer in front. Its boundary is delivery, not any one delivery discipline. Definition comes first and is accepted on a canonical branch. Then a Product Change states what should become true and why. A human validates, applies and accepts it. Then comes delivery, whatever builds: an SDD framework, an AI coding agent given citations as its briefing or a human team receiving a backlog item with its full context. Then come implementation and verification, followed by a proposed Product Change when reality contradicts the definition. The loop closes, whoever did the building, under one rule: one-way authority, two-way learning. Everything delivery learns, including contradictions, partial results and discovered constraints, flows back as proposed changes. Delivery never silently rewrites accepted intent.
