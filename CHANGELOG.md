...
- The specification defines the product model as accepted product intent on the repository's canonical branch. The baseline changes through exactly one operation: a human merging a validated proposed revision. Validation of a proposal is full structural validation of the proposed tree; tools MUST NOT merge a proposal that fails it (RFC #4).
