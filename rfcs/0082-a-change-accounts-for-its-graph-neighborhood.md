...
- **Created:** 2026-08-25
- **Revised:** 2026-08-25 (v4; v2 added impact polarity, per-cause acknowledgments, digest pinning and the apply gate; v3 closed the no-op modify bypass, fixed diagnostic attribution, split PRODUCT002 from PRODUCT033, accounted for product-wide constraints and pinned removals; v4 fixes PRODUCT033 precedence and attribution strings, models product scope as an implicit edge and stages the apply preconditions)
...
**Apply evaluation is staged**, so identical inputs refuse identically everywhere. This RFC fixes where its own diagnostics sit relative to the existing preconditions: (1) change status (`PRODUCT028`); (2) structural and ledger validation of the change (`PRODUCT002`, overlay errors, `PRODUCT033`); (3) baseline drift (`PRODUCT027`); (4) impact causes (`PRODUCT034`). Within a stage every instance is reported, ordered by the standard deterministic sort; a stage with any error stops evaluation, and later stages MUST NOT run or report. The order is by presupposition, not taste: an invalid ledger makes acknowledgment matching meaningless, and causes are computed against the baseline, so a drifted baseline makes the cause set unreliable. `PRODUCT034` therefore fires only when everything it depends on is sound.
