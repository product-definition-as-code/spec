...
Nothing new is known about topologies. What changed is that the vocabulary settled across PRs #9, #14 and #19, and the conformance tests got their first independent runner. Both reasons for waiting are gone, so the clauses below can be written against stable terms and tested rather than asserted.
...
[CONTRIBUTING](../CONTRIBUTING.md) requires every normative statement to be testable, and requires an RFC to say why a clause the conformance tests cannot check is normative anyway. The issue proposed one fixture for the dedicated topology and deferred pointer fixtures. That was checked with `pdac-lint` at `178c13b`, built from source and driving the reference implementation over candidate fixtures, rather than reasoned about.
...
- **On the conformance tests:** one case added, `dedicated-topology`, verified to run and pass. No existing case changes. Pointer cases deferred per Testability.
...
- **Specify the pointer's serialization now, a fixed path and format.** Tempting: it would be testable by a conformance test immediately as a single-repository fixture and would close the interoperability gap. Rejected for v0.1 because no adopter has carried a pointer yet, and a file every consuming repository must agree on should be normalized after a pilot, not invented before one.
...
3. Whether the conformance tests should ever check clause 1 directly, which needs fixtures that are git repositories with history. A test-format question, out of scope here.
