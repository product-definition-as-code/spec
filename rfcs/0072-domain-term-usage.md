...
[`uses-terms`](../spec/relationships.md#canonical-vocabulary) can currently be authored only by a Use Case. That is narrower than the meaning of the `PRODUCT106` warning, "Domain term with no usage", and narrower than the product knowledge the graph is meant to preserve.
...
The Use Case row is unchanged. The other five are added. Actors, Journeys and Bounded Contexts do not gain the field: actors do not themselves state domain semantics; journeys obtain behavioural and domain connections through their use cases; and bounded contexts already own language through the canonical `Domain Term.defined-in` relationship.
...
The follow-up specification change adds valid fixtures with `uses-terms` on each newly permitted source: Business Rule, Domain Term, Functional Requirement, Quality Requirement and Constraint. The fixtures demonstrate that each edge resolves and prevents `PRODUCT106` for its target.
...
- **On existing implementations:** graph relationship declarations, five artifact schemas, reference validation dispatch, `PRODUCT106`, authored templates, graph projections and tests must recognize the expanded source set. Generic reference machinery may be reused, but implementations cannot claim the change needs no validation work.
...
**Add the field to every artifact kind.** Rejected. There is no concrete source semantics for Actors, and Journeys and Bounded Contexts already obtain the relevant connections through relationships designed for them. The vocabulary should expand only where authors need to state a real semantic dependency.

## Consequences

The semantic decision is one coherent edge: `uses-terms` always points from an artifact whose interpretation requires a term to that Domain Term. It applies to Use Cases, Business Rules, Domain Terms, Functional Requirements, Quality Requirements and Constraints. Reverse usage remains derived, prose remains non-structural, and no cycle rule is introduced.
