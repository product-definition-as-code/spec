...
`artifact-kinds-valid` authors a resolving `uses-terms` edge from every permitted source kind. `semantic-uses-terms-unknown` asserts `PRODUCT006` for the new Constraint field.
...
| `PRODUCT106` | domain term with no incoming `uses-terms` relationship | `artifact-kinds-valid` demonstrates every permitted source kind; `structured-behaviour-retired-uses-terms-no-usage` |
