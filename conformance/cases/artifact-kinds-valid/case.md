...
The fixture includes the seven permitted semantic `uses-terms` source kinds. The Use Case, Business Rule, Domain Term, Functional Requirement, Quality Requirement, Constraint and Structured Behaviour each author a resolving edge; every referenced term therefore has a valid incoming use and no `PRODUCT106` warning applies.
