...
`expected.json` asserts two diagnostics: one `PRODUCT025` error per change, against that change's file, with `target` `FR-VALIDATE-001`. Validation is per change - each change's overlay is compiled against the same baseline, and each is validated with the other as concurrent context - so the overlap fails both changes symmetrically: whichever change is rebased or withdrawn, the report told its author. An implementation that reports the overlap against only one of the two changes leaves the other's validation claiming a clean bill it does not have, and fails this case with a missing diagnostic.
